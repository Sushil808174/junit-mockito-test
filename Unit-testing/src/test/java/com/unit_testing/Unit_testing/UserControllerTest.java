...
//        Assertions.assertEquals(allUser.getBody().get(0).getId(), list.get(0).getId());
    ...
    public void testSaveUser_success() throws Exception {
        ...
        Mockito.when(user.getName()).thenReturn("Susheel");
        Mockito.when(user.getMobile()).thenReturn("89849734779");
        Mockito.when(user.getEmail()).thenReturn("sushil@gmail.com");
    ...
    @Test
    public void check_test_failed() throws Exception {
        User user = null;
//        user.setName("Susheel");
//        user.setEmail("sushil@gmail.com");
//        user.setMobile("1234567890");
        Mockito.when(userService.check(user)).thenReturn("Error User not found");
